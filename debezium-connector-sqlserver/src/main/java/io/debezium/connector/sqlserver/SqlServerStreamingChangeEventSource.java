    ...
    public boolean executeIteration(ChangeEventSourceContext context, SqlServerPartition partition, SqlServerOffsetContext offsetContext)
        ...
        try {
            ...
            if (context.isRunning()) {
                ...
                final int maxRowsPerTablePerIteration = 10;
                for (int i = 0; i < tables.length; i++) {
                    changeTables[i] = new SqlServerChangeTablePointer(tables[i], dataConnection, fromLsn, toLsn, maxRowsPerTablePerIteration);
                    changeTables[i].next();
                }
